...
module Diagnostics =
    ...
    type Watchdog
        (
            ...
            zipAgent: MailboxProcessor<FileNameMsg>,
            eventsCounter,
            deletedCounter,
        ...
        let watchdog (_a: System.Timers.ElapsedEventArgs) =
            ...
            let RAMUsage = 
                try
                    (Convert.ToInt32(PC.NextValue()) / (1024)).ToString() //self.PrivateMemorySize64
                with _ -> "NA"
            ...
            diagnosticsAgent.Post(
                Low
                <| sprintf
                    "id %i; cpu: %.1f%%; core: %.1f%%; ram: %s kB; Watch: %i; Read: [%s]; Compress: %i; netQ: %i; ignored: %i"
                    self.Id
                    (CPUUsage
